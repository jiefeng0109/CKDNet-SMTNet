...
def train(training_dbs, validation_db, start_iter=0):
    ...
    with stdout_to_tqdm() as save_stdout:
        for iteration in tqdm(range(start_iter + 1, max_iteration + 1), file=save_stdout, ncols=80):
            ...
            loss,correct,loss1,loss2= nnet.train(**training)
            # training_loss, focal_loss, pull_loss, push_loss, regr_loss, cls_loss = nnet.train(**training)

            if display and iteration % display == 0:
                print("training loss at iteration {}: {}".format(iteration, loss.item()))
                print("training correct at iteration {}: {}".format(iteration, correct.item()))
                print("training cla at iteration {}: {}".format(iteration, loss1.item()))
                print("training reg at iteration {}: {}".format(iteration, loss2.item()))
            ...
            if val_iter and validation_db.db_inds.size and iteration % val_iter == 0:
                ...
                loss,correct,loss1,loss2= nnet.validate(**validation)
                print("validation loss at iteration {}: {}".format(iteration, loss.item()))
                print("validation correct at iteration {}: {}".format(iteration, correct.item()))
                print("training cla at iteration {}: {}".format(iteration, loss1.item()))
                print("training reg at iteration {}: {}".format(iteration, loss2.item()))
                nnet.train_mode()
...
if __name__ == "__main__":
    ...
    training_dbs = [datasets[dataset](train_split) for _ in range(threads)]
    validation_db = datasets[dataset](val_split)
