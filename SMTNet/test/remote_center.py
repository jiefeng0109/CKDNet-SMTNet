...
def testing(nett,result_dir,debug=False):
    path = r'D:/graduate/pro/photo/remote_video/video/4/1960-2690/1960-2690.npz'
    img_path = r'D:/graduate/pro/photo/remote_video/video/4/1960-2690/s/{}.png'
    slabel = np.load(path)['label'].item()
    det_result = np.load('det.npy').item()
    img_len = 6
    # target_ids = ['33']
    target_ids = ['25','28','30','31','80','85','33']
    for target_id in target_ids:
        ...
        for ind in range(25,100,5):
            ...
            for st in idet[dis_index]:
                plot_img(img,st,(0,255,255))
            for st in select_det:
                plot_img(img,st,(255,255,0))
            plot_img(img,track_det2,(0,255,0))
            if target_id in slabel[ind]:
                track_det3 = np.array(slabel[ind][target_id].copy())
                # track_det3[2:4]+=track_det3[:2]
                plot_img(img,track_det3,(0,0,255))
            if cla_track[index]>0.5:
                plot_img(img,track_det1,(255,0,0))


            cv2.imwrite(result_dir+'/{}_{}.png'.format(target_id,ind),img)
            a = 1