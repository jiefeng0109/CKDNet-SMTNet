    ...
    for img_id in range(15,max_img_id+1,5):
        ...
        for target_id,target_list in track_npy[img_id].items():
            if img_id==max_img_id or target_id not in track_npy[next_img_id]:
                if np.argwhere(target_list[:,5]==0).shape[0]>4:
                    target_list[:,4]=max_id
                    target_list[:,2:4] -= target_list[:,:2]
                    track_dict[max_id] = target_list
                    max_id += 1
    write_lines = "{},{},{},{},{},{},-1,-1,-1,-1\n"
    spath = "./results/ct".format(system_configs.mode)
    if not os.path.exists(spath):
        os.makedirs(spath)
    with open(spath+"/{}.txt".format(loc),"w") as f:
...
def testing(nett,result_dir,debug=False):
    locs = ['1460-1400','9590-2960','001']
    det_result = np.load('8_det.npy',allow_pickle=True).item()['det']
    # det_result.update(np.load('9_det.npy',allow_pickle=True).item())
    for loc in locs:
        dets ={}
        offsets = {}
        max_img_id = 0
        for img_names,det in det_result.items():
            if loc in img_names:
                ...
                dets[img_id]=det[:,:5]
                offsets[img_id]=det[:,5:9]
                max_img_id = max(max_img_id,int(img_id))
        import time
        t1 = time.clock()
        track_data = tracker(nett,offsets,dets,max_img_id)
        t2 = time.clock() - t1
        print(loc, len(dets) / t2)
        if len(dets)==0:
            continue
        save_txt(track_data,loc,max_img_id)

    # target_ids = ['33']
def tracker(nett,offsets,det_result,max_img_id,use_predict = True,use_offset=False):
    max_id = 1
    img_id = 10
    re_rate = 10
    ...
    for img_id in range(15,max_img_id+1,5):
        ...
        for tind,target_id in enumerate(pre_target_id):
            ...
            dis = np.sqrt((idet[:,6]-ioff[:,0] - last_p[0])**2+(idet[:,7]-ioff[:,1] - last_p[1])**2)
            select_det_index = np.argsort(dis)[:3]
            temp_dis = dis[select_det_index]
            temp_ind = np.argwhere(temp_dis<15)[:,0]
            select_det_index = select_det_index[temp_ind]
            if select_det_index.shape[0]==0:
                select_det = target_list[-1:]
            else:
                # dis_temp = dis[select_det_index]
                # for i,ind_ds in enumerate(dis_temp):
                #     if ind_ds>10:
                #         select_det_index=select_det_index[:i]
                #         break
            ...
            tspace = torch.from_numpy(tspace.astype(np.float32))
        ...
        space_list = torch.clamp(torch.cat(space_list,dim=0),min=-20,max=20)
        xs = [None,lstm_list/re_rate,space_list/re_rate]
        ...
        for tind,target_id in enumerate(pre_target_id):
            ...
            # temp_reg = np.mean(reg_track,axis=0)
            temp_reg = np.mean(reg_track,axis=0)*re_rate
