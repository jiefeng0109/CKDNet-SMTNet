        ...
        for target_id,target_list in track_npy[img_id].items():
            if img_id==max_img_id or target_id not in track_npy[next_img_id]:
                if np.argwhere(target_list[:,5]==0).shape[0]>2:
                    ...
                    max_id += 1
    write_lines = "{},{},{},{},{},{},-1,-1,-1,-1\n"
    spath = "./results/crn".format(system_configs.mode)
...
def testing(nett,result_dir,debug=False):
    ...
    det_result.update(np.load('9_det.npy',allow_pickle=True).item()['det'])
    for loc in locs:
        dets ={}
        max_img_id = 0
        ...
        t2 = time.clock() - t1
        print(loc, len(dets),t2,len(dets) / t2)
        if len(dets)==0:
            continue
        save_txt(track_data,loc,max_img_id)

    # target_ids = ['33']
def tracker(nett,slabel,det_result,max_img_id,use_predict = True):
    max_id = 1
    img_id = 10
    re_rate = 10
