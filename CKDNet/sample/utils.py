...
def draw_gaussian(heatmap, center, radius,gaussian=None , k=1, delte=6):
    if gaussian==None:
        diameter = 2 * radius + 1
...
def draw_offset(heatmap, center, radius, k=1):
    ...
    masked_heatmap = heatmap[y - top:y + bottom, x - left:x + right]
    masked_offset = np.ones_like(masked_heatmap)
    np.maximum(masked_heatmap, masked_offset * k / 20, out=masked_heatmap)
def draw_weight(heatmap, center, radius, k=1):
    ...
    masked_heatmap = heatmap[y - top:y + bottom, x - left:x + right]
    masked_offset = np.ones_like(masked_heatmap)
    np.maximum(masked_heatmap, masked_offset * k, out=masked_heatmap)
...
def random_crop(image, detections, random_scales, view_size, border=64):
    ...
    image_height, image_width, channl = image.shape[0:3]

    scale  = np.random.choice(random_scales)
    height = int(view_height * scale)
    width  = int(view_width  * scale)

    cropped_image = np.zeros((height, width, channl), dtype=image.dtype)
