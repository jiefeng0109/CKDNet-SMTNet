...
__global__ void left_pooling_forward_kernel(int N,  float const *x, int w, int h, int c, int batch, float *offset, float *forward_ind, float *out)
{
...
__global__ void left_pooling_backward_kernel(int N,  float const *x, int w, int h, int c, int batch, float *backward_ind, float *out)
{
    int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(i >= N) return;
...
void left_pooling_forward_ongpu(float *x, int w, int h, int c, int batch, float *offset, float *forward_ind, float *out)
{
    ...
    cudaError_t err;
    left_pooling_forward_kernel<<<cuda_gridsize(size), BLOCK>>>(size, x, w, h, c, batch, offset, forward_ind, out);
...
void left_pooling_backward_ongpu(float *x, int w, int h, int c, int batch, float *backward_ind, float *out)
{
    int size = w*h*c*batch;
    cudaError_t err;
    left_pooling_backward_kernel<<<cuda_gridsize(size), BLOCK>>>(size, x, w, h, c, batch, backward_ind, out);
