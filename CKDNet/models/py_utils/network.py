...
class ResNet_top(nn.Module):
    ...
    def __init__(self):
        ...
        self.pre = nn.Sequential(
            conv_bn_relu(3, 64, kernel_size=3, stride=1, padding=1, has_bn=True, has_relu=True),
            conv_bn_relu(64, 64, kernel_size=3, stride=1, padding=1, has_bn=True, has_relu=True),
            conv_bn_relu(64, 128, kernel_size=3, stride=2, padding=1, has_bn=True, has_relu=True),
            conv_bn_relu(128, 64, kernel_size=3, stride=1, padding=1, has_bn=True, has_relu=True),
            # conv_bn_relu(64, 64, kernel_size=3, stride=2, padding=1, has_bn=True, has_relu=True)
        )
        self.diff1 = Diff_cuda()
        ...
        #     conv_bn_relu(128, 64, kernel_size=3, stride=1, padding=1, has_bn=True, has_relu=True)
        # )
        self.conv_1 = conv_bn_relu(64*3, 64, kernel_size=3, stride=1, padding=1, has_bn=True, has_relu=True)
        self.conv_2 = conv_bn_relu(128, 64, kernel_size=3, stride=1, padding=1, has_bn=True, has_relu=True)
        self.conv_3 = conv_bn_relu(64, 64, kernel_size=3, stride=1, padding=1, has_bn=True, has_relu=True)
    ...
    def forward(self, x):
        ...
        x3 = self.pre(x[:,6:9])
        x2 = self.pre(x[:,3:6])
        dx1 = self.diff1(x1,x2)
        dx2 = self.diff2(x2,x3)
        dx3 = self.diff3(x1,x3)
...
class MSPN(nn.Module):
    ...
    def __init__(self, STAGE_NUM=1,OUTPUT_SHAPE=[128,128],OUTPUT_CHANNEL_NUM=256, UPSAMPLE_CHANNEL_NUM = 256,run_efficient=False, **kwargs):
        super(MSPN, self).__init__()
        self.top = ResNet_top()
        self.stage_num = STAGE_NUM
        # self.output_chl_num = cfg.DATASET.KEYPOINT.NUM
        self.output_chl_num = OUTPUT_CHANNEL_NUM
        self.output_shape = OUTPUT_SHAPE
        self.upsample_chl_num = UPSAMPLE_CHANNEL_NUM
