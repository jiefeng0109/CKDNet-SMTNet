...
class CervicalDataset(object):
    ...
    def __init__(self, data_path, batch_size=1,patch_size=[256,256], transform=None):
    ...
    def crop_patch(self, img, labels,img_num):
        ...
        nxs = np.random.uniform(0,W-px,100).astype(np.int)
        nys = np.random.uniform(0,H-py,100).astype(np.int)
        for nxy in zip(nxs,nys):
